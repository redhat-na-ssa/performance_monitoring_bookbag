...
[subs=-macros]
3. Copy this URL: `{hyperfoil_benchmark_definition_url}`, paste in the text box, and click the `Upload from URL` button.
...
Our Openshift Cluster has the *Serverless capability* enabled, as such, our applications get deployed as Serverless workloads leveraging all the
...
So far we generated small traffic using our load test driver (Hyperfoil), just enough to generate some resource consumption metrics.
...
As our applications provide endpoints to load either the CPU or the memory, we will explore usage of the _CPU_ and _Memory usage_ 
...
If you get an authorization error, use the Task `98: OC Login` in your DevWorkspace to login with your OpenShift credentials.
